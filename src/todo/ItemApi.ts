...
import ItemProps from './ItemProps'; // Importing Item interface from ItemProps
...
export const updateItem: (token: string, item: ItemProps) => Promise<ItemProps> = (token, item) => {
  return withLogs(axios.put(`${itemUrl}/${item.id}`, item, authConfig(token)), 'updateItem'); // Using `id` instead of `_id`
}
